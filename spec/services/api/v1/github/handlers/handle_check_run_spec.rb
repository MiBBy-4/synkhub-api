...
RSpec.describe Api::V1::Github::Handlers::HandleCheckRun do
  describe ".call" do
    ...
    let(:check_name) { "tests" }
    let(:conclusion) { "success" }
    let(:check_url) { "https://github.com/#{repo_full_name}/runs/1" }
    let(:event) do
      create(:github_webhook_event,
             ...
             payload: {
               ...
               "sender" => { "login" => actor },
               "check_run" => { "name" => check_name, "conclusion" => conclusion, "html_url" => check_url },
             })
    end
    ...
    it "creates a notification with correct title and url" do
      ...
      expect(notification.title).to eq("Check '#{check_name}' completed (#{conclusion}) in #{repo_full_name}")
      expect(notification.url).to eq(check_url)
    end
  end
end
