...
RSpec.describe Api::V1::Github::Handlers::HandleCreate do
  describe ".call" do
    ...
    context "when branch is created" do
      it "creates notification with branch title" do
        ...
        expect(notification.title).to eq("#{actor} created branch #{ref} in #{repo_full_name}")
        expect(notification.url).to eq("https://github.com/#{repo_full_name}/tree/#{ref}")
      end
    end
    ...
    context "when tag is created" do
      ...
      it "creates notification with tag title" do
        expect(result).to be_success
        expect(GithubNotification.last.title).to eq("#{actor} created tag #{ref} in #{repo_full_name}")
      end
    end
  end
end
